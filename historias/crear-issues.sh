...
# Uso:  bash historias/crear-issues.sh [owner/repo]

set -euo pipefail
REPO="${1:-donaldoAndres1985/Regenta}"
echo "Repositorio: $REPO"

echo "== Etiquetas =="
gh label create "alertas" --repo "$REPO" --color "8A5B06" --force >/dev/null && echo "  alertas"
gh label create "auditoria" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  auditoria"
gh label create "backend" --repo "$REPO" --color "1D4E6B" --force >/dev/null && echo "  backend"
gh label create "bbdd" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  bbdd"
gh label create "caja" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  caja"
gh label create "ci" --repo "$REPO" --color "5E7183" --force >/dev/null && echo "  ci"
gh label create "clave" --repo "$REPO" --color "9A5709" --force >/dev/null && echo "  clave"
gh label create "clientes" --repo "$REPO" --color "0C6473" --force >/dev/null && echo "  clientes"
gh label create "comandas" --repo "$REPO" --color "A03325" --force >/dev/null && echo "  comandas"
gh label create "compras" --repo "$REPO" --color "9A5709" --force >/dev/null && echo "  compras"
gh label create "core" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  core"
gh label create "eventos" --repo "$REPO" --color "8A5B06" --force >/dev/null && echo "  eventos"
gh label create "facturacion" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  facturacion"
gh label create "flutter" --repo "$REPO" --color "0C6473" --force >/dev/null && echo "  flutter"
gh label create "fundacion" --repo "$REPO" --color "6E4B1F" --force >/dev/null && echo "  fundacion"
gh label create "infra" --repo "$REPO" --color "5E7183" --force >/dev/null && echo "  infra"
gh label create "inventario" --repo "$REPO" --color "9A5709" --force >/dev/null && echo "  inventario"
gh label create "menu" --repo "$REPO" --color "A03325" --force >/dev/null && echo "  menu"
gh label create "mesas" --repo "$REPO" --color "A03325" --force >/dev/null && echo "  mesas"
gh label create "recursos" --repo "$REPO" --color "0C6473" --force >/dev/null && echo "  recursos"
gh label create "reportes" --repo "$REPO" --color "4A5C6D" --force >/dev/null && echo "  reportes"
gh label create "reservas" --repo "$REPO" --color "0C6473" --force >/dev/null && echo "  reservas"
gh label create "seguridad" --repo "$REPO" --color "A0271B" --force >/dev/null && echo "  seguridad"
gh label create "ventas" --repo "$REPO" --color "9A5709" --force >/dev/null && echo "  ventas"

echo "== Hitos (milestones) =="
gh api "repos/$REPO/milestones" -f title="E00 · Fundación" -f description="Dejar los dos repos, las bases de datos y el CI en pie para que cualquier módulo posterior arranque sin decisiones pendientes." >/dev/null 2>&1 || true
echo "  E00 · Fundación"
gh api "repos/$REPO/milestones" -f title="E01 · Core · Tenant e identidad" -f description="El registro de negocios, los planes, los usuarios y el motor de permisos. Nada funciona sin esto." >/dev/null 2>&1 || true
echo "  E01 · Core · Tenant e identidad"
gh api "repos/$REPO/milestones" -f title="E02 · Clientes y cartera" -f description="El CRM que consumen los tres patrones y la cartera de crédito." >/dev/null 2>&1 || true
echo "  E02 · Clientes y cartera"
gh api "repos/$REPO/milestones" -f title="E03 · Inventario · catálogo del patrón Venta directa" -f description="El catálogo configurable, el stock por bodega y el libro mayor de movimientos. Aquí vive la generalización por configuración." >/dev/null 2>&1 || true
echo "  E03 · Inventario · catálogo del patrón Venta directa"
gh api "repos/$REPO/milestones" -f title="E04 · Ventas · transacción del patrón Venta directa" -f description="El POS, la saga con Inventario, los pagos y las devoluciones." >/dev/null 2>&1 || true
echo "  E04 · Ventas · transacción del patrón Venta directa"
gh api "repos/$REPO/milestones" -f title="E05 · Compras y proveedores" -f description="Lo que llena el inventario. Sin esto, el stock solo baja." >/dev/null 2>&1 || true
echo "  E05 · Compras y proveedores"
gh api "repos/$REPO/milestones" -f title="E06 · Facturación electrónica DIAN" -f description="Emisión, firma, transmisión y notas crédito. Sirve a los tres patrones." >/dev/null 2>&1 || true
echo "  E06 · Facturación electrónica DIAN"
gh api "repos/$REPO/milestones" -f title="E07 · Caja y POS" -f description="Apertura, arqueo y cierre. Transversal a los tres patrones." >/dev/null 2>&1 || true
echo "  E07 · Caja y POS"
gh api "repos/$REPO/milestones" -f title="E08 · Recursos · catálogo del patrón Reserva" -f description="Los recursos reservables, sus atributos configurables y las tarifas." >/dev/null 2>&1 || true
echo "  E08 · Recursos · catálogo del patrón Reserva"
gh api "repos/$REPO/milestones" -f title="E09 · Reservas · transacción del patrón Reserva" -f description="La reserva sobre un intervalo de tiempo, el check-in, los consumos y el cierre." >/dev/null 2>&1 || true
echo "  E09 · Reservas · transacción del patrón Reserva"
gh api "repos/$REPO/milestones" -f title="E10 · Menú · catálogo del patrón Comanda" -f description="La carta, los modificadores y las recetas que unen la comanda con el inventario." >/dev/null 2>&1 || true
echo "  E10 · Menú · catálogo del patrón Comanda"
gh api "repos/$REPO/milestones" -f title="E11 · Mesas · el salón" -f description="Zonas, mesas y sesiones de mesa. Sin equivalente en los otros patrones." >/dev/null 2>&1 || true
echo "  E11 · Mesas · el salón"
gh api "repos/$REPO/milestones" -f title="E12 · Comandas · transacción del patrón Comanda" -f description="El pedido que queda abierto, con estado por línea, cocina y división de cuenta." >/dev/null 2>&1 || true
echo "  E12 · Comandas · transacción del patrón Comanda"
gh api "repos/$REPO/milestones" -f title="E13 · Alertas y notificaciones" -f description="Un motor de reglas, no condiciones escritas en el código." >/dev/null 2>&1 || true
echo "  E13 · Alertas y notificaciones"
gh api "repos/$REPO/milestones" -f title="E14 · Reportes y dashboards" -f description="Un read model alimentado por eventos, no vistas sobre otras bases." >/dev/null 2>&1 || true
echo "  E14 · Reportes y dashboards"
gh api "repos/$REPO/milestones" -f title="E15 · Auditoría y sincronización" -f description="Dos problemas distintos que el diseño original junta: la bitácora de quién cambió qué, y la cola de operaciones del cliente offline." >/dev/null 2>&1 || true
echo "  E15 · Auditoría y sincronización"
gh api "repos/$REPO/milestones" -f title="E16 · App Flutter · núcleo" -f description="El tema, la navegación, la sesión y la base offline que comparten todos los módulos." >/dev/null 2>&1 || true
echo "  E16 · App Flutter · núcleo"

echo "== Historias =="
gh issue create --repo "$REPO" --title 'HU-001 · Scaffolding del monorepo de backend' --milestone 'E00 · Fundación' --label "fundacion,backend" --body '**Como** desarrollador, **quiero** tener el repo `regenta-backend` con un módulo Maven por microservicio y un POM padre **para** que agregar un servicio nuevo sea copiar una carpeta y no rediscutir la estructura
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-001'
gh issue create --repo "$REPO" --title 'HU-002 · Scaffolding del monorepo Flutter con melos' --milestone 'E00 · Fundación' --label "fundacion,flutter" --body '**Como** desarrollador, **quiero** tener `regenta-app` como monorepo melos con un paquete por módulo y una sola app **para** que la app cargue solo los paquetes del patrón operativo del negocio
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-002'
gh issue create --repo "$REPO" --title 'HU-003 · Levantar el entorno local con Docker Compose' --milestone 'E00 · Fundación' --label "fundacion,infra" --body '**Como** desarrollador, **quiero** levantar PostgreSQL, RabbitMQ y los servicios con un solo comando **para** poder trabajar sin depender de infraestructura desplegada
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-003'
gh issue create --repo "$REPO" --title 'HU-004 · Crear una base de datos por microservicio' --milestone 'E00 · Fundación' --label "fundacion,bbdd" --body '**Como** arquitecto, **quiero** que cada microservicio tenga su propia base, nunca una compartida **para** que ningún servicio pueda leer las tablas de otro ni por error
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-004'
gh issue create --repo "$REPO" --title 'HU-005 · Migraciones versionadas con Flyway por servicio' --milestone 'E00 · Fundación' --label "fundacion,bbdd" --body '**Como** desarrollador, **quiero** que el esquema de cada servicio se cree y evolucione con migraciones versionadas **para** que el esquema en local, en CI y en producción sea siempre el mismo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-006'
gh issue create --repo "$REPO" --title 'HU-007 · API Gateway con validación de JWT y enrutamiento' --milestone 'E00 · Fundación' --label "fundacion,backend,seguridad" --body '**Como** arquitecto, **quiero** un único punto de entrada que valide el token y enrute a cada microservicio **para** no duplicar la lógica de autenticación en cada servicio ni en cada cliente
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-007'
gh issue create --repo "$REPO" --title 'HU-008 · Outbox e Inbox como librería compartida' --milestone 'E00 · Fundación' --label "fundacion,backend,eventos" --body '**Como** desarrollador, **quiero** una librería con el patrón Outbox e Inbox que todo servicio pueda usar **para** que ningún evento se pierda cuando el bus está caído ni se procese dos veces cuando llega repetido
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-008'
gh issue create --repo "$REPO" --title 'HU-009 · Pipeline de CI para ambos repos' --milestone 'E00 · Fundación' --label "fundacion,ci" --body '**Como** desarrollador, **quiero** que cada push y cada PR compile y ejecute los tests automáticamente **para** no descubrir en producción algo que un test hubiera atrapado
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-009'
gh issue create --repo "$REPO" --title 'HU-010 · Documentación OpenAPI por servicio' --milestone 'E00 · Fundación' --label "fundacion,backend" --body '**Como** desarrollador de la app, **quiero** que cada servicio publique su contrato OpenAPI actualizado **para** poder generar el cliente Dart sin adivinar la forma de las respuestas
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-010'
gh issue create --repo "$REPO" --title 'HU-011 · Registrar un negocio nuevo con su plan y patrón' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend" --body '**Como** operador de Regenta, **quiero** dar de alta un negocio eligiendo plan y patrón operativo **para** vender a un cliente nuevo sin desplegar nada
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-011'
gh issue create --repo "$REPO" --title 'HU-012 · Crear el primer usuario administrador del negocio' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend" --body '**Como** operador de Regenta, **quiero** que al crear un negocio se cree su primer usuario administrador **para** que el cliente pueda entrar el mismo día sin intervención nuestra
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-012'
gh issue create --repo "$REPO" --title 'HU-013 · Autenticación con emisión de JWT' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,seguridad" --body '**Como** usuario del negocio, **quiero** entrar con mi correo y contraseña y recibir un token **para** poder usar la app desde el celular y desde el navegador
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-013'
gh issue create --repo "$REPO" --title 'HU-014 · Rotación y revocación de refresh tokens' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,seguridad" --body '**Como** responsable de seguridad, **quiero** que el refresh token rote en cada uso y se pueda revocar **para** que un token robado tenga ventana corta y se pueda cortar el acceso de un dispositivo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-014'
gh issue create --repo "$REPO" --title 'HU-015 · Gestión de usuarios del negocio con límite por plan' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,flutter" --body '**Como** administrador del negocio, **quiero** invitar, editar y desactivar usuarios de mi negocio **para** controlar quién entra al sistema sin depender de soporte
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-015'
gh issue create --repo "$REPO" --title 'HU-016 · Motor de roles y permisos por negocio' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,flutter" --body '**Como** administrador del negocio, **quiero** definir roles con permisos específicos **para** que el vendedor no pueda editar precios ni ver los costos
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-016'
gh issue create --repo "$REPO" --title 'HU-017 · Validación de módulo por plan en el backend' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,seguridad" --body '**Como** arquitecto, **quiero** una anotación que bloquee el acceso a un módulo que el plan no incluye **para** que ocultar el botón en la app no sea la única defensa
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-017'
gh issue create --repo "$REPO" --title 'HU-018 · Configuración fiscal y de operación del negocio' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,flutter" --body '**Como** administrador del negocio, **quiero** configurar mis datos fiscales, impuestos y preferencias **para** que las facturas salgan con mis datos y los precios calculen bien
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-018'
gh issue create --repo "$REPO" --title 'HU-019 · Sucursales y bodegas del negocio' --milestone 'E01 · Core · Tenant e identidad' --label "core,backend,flutter" --body '**Como** administrador del negocio, **quiero** administrar mis sucursales **para** poder operar en más de un punto físico
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-019'
gh issue create --repo "$REPO" --title 'HU-020 · Consulta del plan y los módulos activos desde la app' --milestone 'E01 · Core · Tenant e identidad' --label "core,flutter" --body '**Como** usuario del negocio, **quiero** que la app sepa qué módulos tengo activos **para** que no me muestre pantallas que mi plan no incluye
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-020'
gh issue create --repo "$REPO" --title 'HU-021 · Crear y consultar clientes' --milestone 'E02 · Clientes y cartera' --label "clientes,backend,flutter" --body '**Como** vendedor, **quiero** registrar clientes con sus datos fiscales y de contacto **para** poder facturarles y llevar su historial
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-021'
gh issue create --repo "$REPO" --title 'HU-022 · Cupo de crédito y cartera del cliente' --milestone 'E02 · Clientes y cartera' --label "clientes,backend,flutter" --body '**Como** administrador del negocio, **quiero** asignar cupo de crédito y ver el saldo de cada cliente **para** saber a quién puedo venderle a plazo y a quién no
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-022'
gh issue create --repo "$REPO" --title 'HU-023 · Métricas del cliente alimentadas por eventos' --milestone 'E02 · Clientes y cartera' --label "clientes,backend,eventos" --body '**Como** gerente, **quiero** ver cuánto ha comprado un cliente y cuándo fue su última compra **para** saber a quién vale la pena llamar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-023'
gh issue create --repo "$REPO" --title 'HU-024 · Historial de interacciones con el cliente' --milestone 'E02 · Clientes y cartera' --label "clientes,backend,flutter" --body '**Como** vendedor, **quiero** registrar llamadas, visitas y notas sobre un cliente **para** que la siguiente persona que lo atienda sepa qué se habló
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-024'
gh issue create --repo "$REPO" --title 'HU-025 · Listado de clientes en móvil con búsqueda y filtros' --milestone 'E02 · Clientes y cartera' --label "clientes,flutter" --body '**Como** vendedor en mostrador, **quiero** buscar un cliente rápido desde el celular **para** no demorar la venta buscando a quién facturar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-025'
gh issue create --repo "$REPO" --title 'HU-026 · Categorías del negocio con jerarquía' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,flutter" --body '**Como** administrador del negocio, **quiero** crear mis propias categorías de producto **para** organizar el catálogo con las palabras de mi negocio, no con las del sistema
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-026'
gh issue create --repo "$REPO" --title 'HU-027 · Definir los atributos que exige cada categoría' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,flutter,clave" --body '**Como** administrador del negocio, **quiero** decidir qué campos extra pide un producto según su categoría **para** que una droguería exija lote y vencimiento sin que nadie programe nada
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-027'
gh issue create --repo "$REPO" --title 'HU-028 · Crear producto con validación de atributos dinámicos' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,flutter,clave" --body '**Como** administrador del negocio, **quiero** crear un producto y que el sistema me exija los campos de su categoría **para** no tener que pedir un desarrollo cada vez que aparece un tipo de producto nuevo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-028'
gh issue create --repo "$REPO" --title 'HU-029 · Bodegas y existencias por producto y bodega' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,clave" --body '**Como** administrador del negocio, **quiero** saber cuánto tengo de cada producto en cada bodega **para** no vender lo que está en otra sede o ya está apartado
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-029'
gh issue create --repo "$REPO" --title 'HU-030 · Libro mayor de movimientos de inventario' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,clave" --body '**Como** auditor, **quiero** que todo cambio de stock quede registrado y no se pueda alterar **para** poder explicar cualquier descuadre sin adivinar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-030'
gh issue create --repo "$REPO" --title 'HU-031 · Lotes y fechas de vencimiento' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend" --body '**Como** administrador de droguería, **quiero** controlar lotes con su fecha de vencimiento **para** cumplir la norma sanitaria y no vender producto vencido
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-031'
gh issue create --repo "$REPO" --title 'HU-032 · Traslados entre bodegas' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend" --body '**Como** administrador del negocio, **quiero** mover mercancía de una bodega a otra con trazabilidad **para** que lo que sale de una sede y aún no llega a la otra no desaparezca del inventario
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-032'
gh issue create --repo "$REPO" --title 'HU-033 · Ajustes de inventario con motivo' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend" --body '**Como** administrador del negocio, **quiero** ajustar el stock tras un conteo físico o una avería **para** que el sistema refleje lo que hay de verdad, con constancia de por qué cambió
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-033'
gh issue create --repo "$REPO" --title 'HU-034 · Reserva y liberación de stock para la saga de ventas' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend,eventos,clave" --body '**Como** arquitecto, **quiero** que Inventario pueda apartar stock temporalmente y liberarlo si la venta no se concreta **para** que dos cajas no vendan la última unidad al mismo tiempo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-034'
gh issue create --repo "$REPO" --title 'HU-035 · Búsqueda de productos y escaneo de código de barras' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,flutter" --body '**Como** vendedor, **quiero** encontrar un producto por nombre, SKU o escaneando su código **para** no demorar la venta buscando en una lista larga
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-035'
gh issue create --repo "$REPO" --title 'HU-036 · Listas de precios y precios por volumen' --milestone 'E03 · Inventario · catálogo del patrón Venta directa' --label "inventario,backend" --body '**Como** administrador del negocio, **quiero** manejar precios distintos por lista de precios y por cantidad **para** poder vender al mayorista a otro precio sin duplicar el catálogo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-036'
gh issue create --repo "$REPO" --title 'HU-037 · Crear una venta en borrador con sus líneas' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend,clave" --body '**Como** vendedor, **quiero** armar una venta agregando productos con su cantidad y precio **para** poder cobrar lo que el cliente lleva
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-037'
gh issue create --repo "$REPO" --title 'HU-038 · Saga de confirmación de venta con reserva de stock' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend,eventos,clave" --body '**Como** arquitecto, **quiero** que confirmar una venta reserve el stock y compense si no alcanza **para** que no exista una venta confirmada sin stock descontado ni stock descontado sin venta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-038'
gh issue create --repo "$REPO" --title 'HU-039 · Registrar el pago de una venta, incluso mixto' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend,flutter" --body '**Como** cajero, **quiero** cobrar una venta con uno o varios medios de pago **para** poder aceptar que alguien pague una parte en efectivo y otra con tarjeta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-039'
gh issue create --repo "$REPO" --title 'HU-040 · Venta a crédito con validación de cupo' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend" --body '**Como** vendedor, **quiero** vender a plazo a un cliente con cupo aprobado **para** no perder la venta del cliente habitual que paga a 30 días
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-040'
gh issue create --repo "$REPO" --title 'HU-041 · Anular una venta con reintegro de stock' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend" --body '**Como** administrador del negocio, **quiero** anular una venta mal registrada **para** corregir un error sin que el inventario quede mal
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-041'
gh issue create --repo "$REPO" --title 'HU-042 · Devoluciones totales y parciales' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend,flutter" --body '**Como** vendedor, **quiero** recibir la devolución de parte de una venta **para** atender al cliente que devuelve solo uno de los productos que llevó
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-042'
gh issue create --repo "$REPO" --title 'HU-043 · Sincronización de ventas creadas sin conexión' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend,flutter,clave" --body '**Como** vendedor en ruta, **quiero** registrar ventas sin señal y que suban solas al recuperar conexión **para** no perder la venta ni tener que volver a digitarla
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-043'
gh issue create --repo "$REPO" --title 'HU-044 · Cotizaciones que se convierten en venta' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,backend" --body '**Como** vendedor, **quiero** hacer una cotización y luego convertirla en venta **para** no volver a digitar todo cuando el cliente acepta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-044'
gh issue create --repo "$REPO" --title 'HU-045 · Pantalla de POS en móvil y en web' --milestone 'E04 · Ventas · transacción del patrón Venta directa' --label "ventas,flutter" --body '**Como** vendedor, **quiero** armar y cobrar una venta desde el celular o desde el computador **para** atender igual de rápido en el mostrador y en la oficina
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-045'
gh issue create --repo "$REPO" --title 'HU-046 · Administrar proveedores' --milestone 'E05 · Compras y proveedores' --label "compras,backend,flutter" --body '**Como** administrador del negocio, **quiero** registrar mis proveedores con sus condiciones **para** saber a quién comprarle y con qué plazo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-046'
gh issue create --repo "$REPO" --title 'HU-047 · Órdenes de compra con aprobación' --milestone 'E05 · Compras y proveedores' --label "compras,backend,flutter" --body '**Como** administrador del negocio, **quiero** crear órdenes de compra y aprobarlas antes de enviarlas **para** controlar el gasto antes de comprometerlo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-047'
gh issue create --repo "$REPO" --title 'HU-048 · Recepción de mercancía con captura de lotes' --milestone 'E05 · Compras y proveedores' --label "compras,backend,flutter,clave" --body '**Como** bodeguero, **quiero** recibir la mercancía de una orden y capturar lote y vencimiento donde aplique **para** que el inventario refleje lo que llegó de verdad, con su trazabilidad sanitaria
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-048'
gh issue create --repo "$REPO" --title 'HU-049 · Cuentas por pagar y pagos a proveedores' --milestone 'E05 · Compras y proveedores' --label "compras,backend" --body '**Como** administrador del negocio, **quiero** llevar lo que le debo a cada proveedor y registrar los pagos **para** no quedar mal con un proveedor por olvido
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-049'
gh issue create --repo "$REPO" --title 'HU-050 · Sugerencia de compra a partir de stock bajo mínimo' --milestone 'E05 · Compras y proveedores' --label "compras,backend" --body '**Como** administrador del negocio, **quiero** que el sistema me sugiera qué reponer **para** no descubrir que se acabó algo cuando el cliente ya lo está pidiendo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-050'
gh issue create --repo "$REPO" --title 'HU-051 · Recepción desde el celular en la bodega' --milestone 'E05 · Compras y proveedores' --label "compras,flutter" --body '**Como** bodeguero, **quiero** recibir mercancía desde el celular, parado junto a las cajas **para** no tener que anotar en papel y digitar después
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-051'
gh issue create --repo "$REPO" --title 'HU-052 · Administrar resoluciones de numeración DIAN' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend" --body '**Como** administrador del negocio, **quiero** cargar mis resoluciones de facturación con su rango **para** poder facturar legalmente y saber cuándo se me está agotando el rango
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-052'
gh issue create --repo "$REPO" --title 'HU-053 · Emitir factura desde cualquiera de los tres patrones' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend,eventos,clave" --body '**Como** arquitecto, **quiero** que Facturación emita a partir de una venta, una estancia o una comanda **para** no tener que reescribir el servicio cuando se activa otro patrón
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-053'
gh issue create --repo "$REPO" --title 'HU-054 · Asignación del consecutivo dentro del rango autorizado' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend,clave" --body '**Como** contador, **quiero** que la numeración sea continua y no se salga del rango **para** que la DIAN no rechace la facturación por huecos o números fuera de rango
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-054'
gh issue create --repo "$REPO" --title 'HU-055 · Firma digital y transmisión a la DIAN' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend,seguridad" --body '**Como** contador, **quiero** que la factura se firme y se transmita, y quede constancia de la respuesta **para** poder demostrar qué se envió y qué respondieron
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-055'
gh issue create --repo "$REPO" --title 'HU-056 · Notas crédito' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend" --body '**Como** contador, **quiero** emitir una nota crédito contra una factura **para** poder anular o corregir legalmente una factura ya aceptada
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-056'
gh issue create --repo "$REPO" --title 'HU-057 · Modo de contingencia cuando la DIAN no responde' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,backend" --body '**Como** contador, **quiero** poder seguir facturando cuando el servicio de la DIAN está caído **para** no tener que cerrar el negocio porque un tercero falló
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-057'
gh issue create --repo "$REPO" --title 'HU-058 · Consulta y envío de facturas al cliente' --milestone 'E06 · Facturación electrónica DIAN' --label "facturacion,flutter" --body '**Como** cajero, **quiero** ver una factura emitida, imprimirla y enviarla por correo **para** entregarle al cliente su comprobante como lo pida
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-058'
gh issue create --repo "$REPO" --title 'HU-059 · Abrir y cerrar sesión de caja' --milestone 'E07 · Caja y POS' --label "caja,backend,flutter" --body '**Como** cajero, **quiero** abrir mi caja con una base y cerrarla declarando lo que conté **para** que quede claro de quién es la responsabilidad del efectivo del turno
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-059'
gh issue create --repo "$REPO" --title 'HU-060 · Movimientos de caja desde los tres patrones' --milestone 'E07 · Caja y POS' --label "caja,backend,eventos" --body '**Como** cajero, **quiero** que todo cobro entre a mi caja sin importar de dónde venga **para** que el arqueo cuadre aunque el negocio sea un restaurante o un hotel
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-060'
gh issue create --repo "$REPO" --title 'HU-061 · Ingresos, retiros y gastos de caja' --milestone 'E07 · Caja y POS' --label "caja,backend,flutter" --body '**Como** cajero, **quiero** registrar retiros de efectivo y gastos menores **para** que el arqueo cuadre cuando saco plata para consignar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-061'
gh issue create --repo "$REPO" --title 'HU-062 · Arqueo por denominaciones' --milestone 'E07 · Caja y POS' --label "caja,backend,flutter" --body '**Como** cajero, **quiero** contar el efectivo por billetes y monedas al cerrar **para** no equivocarme sumando de cabeza
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-062'
gh issue create --repo "$REPO" --title 'HU-063 · Reporte de cierre de caja' --milestone 'E07 · Caja y POS' --label "caja,backend,flutter" --body '**Como** gerente, **quiero** ver el resumen del turno de cada caja **para** saber si hubo descuadres y de quién fue el turno
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-063'
gh issue create --repo "$REPO" --title 'HU-064 · Tipos de recurso con atributos configurables' --milestone 'E08 · Recursos · catálogo del patrón Reserva' --label "recursos,backend,flutter" --body '**Como** administrador del hotel, **quiero** definir mis tipos de recurso y qué campos describe cada uno **para** que una habitación, una cancha y un consultorio usen el mismo sistema
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-064'
gh issue create --repo "$REPO" --title 'HU-065 · Administrar recursos individuales' --milestone 'E08 · Recursos · catálogo del patrón Reserva' --label "recursos,backend,flutter" --body '**Como** administrador del hotel, **quiero** dar de alta cada habitación, cancha o consultorio **para** poder reservarlos uno por uno
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-065'
gh issue create --repo "$REPO" --title 'HU-066 · Tarifas por temporada, día y franja con prioridad' --milestone 'E08 · Recursos · catálogo del patrón Reserva' --label "recursos,backend,flutter" --body '**Como** administrador del hotel, **quiero** definir tarifas distintas por temporada y día de la semana **para** cobrar distinto un martes de mayo que un sábado de diciembre
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-066'
gh issue create --repo "$REPO" --title 'HU-067 · Bloqueos de recurso por mantenimiento' --milestone 'E08 · Recursos · catálogo del patrón Reserva' --label "recursos,backend" --body '**Como** administrador del hotel, **quiero** bloquear un recurso por un periodo **para** que no se reserve una habitación que está en obra
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-067'
gh issue create --repo "$REPO" --title 'HU-068 · Servicios adicionales y políticas de cancelación' --milestone 'E08 · Recursos · catálogo del patrón Reserva' --label "recursos,backend,flutter" --body '**Como** administrador del hotel, **quiero** ofrecer desayuno, parqueadero y demás, y fijar mis políticas de cancelación **para** cobrar los extras y saber cuánto retengo si cancelan
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-068'
gh issue create --repo "$REPO" --title 'HU-069 · Consultar disponibilidad en un periodo' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend" --body '**Como** recepcionista, **quiero** ver qué recursos están libres entre dos fechas **para** poder responderle al cliente que llama preguntando si hay
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-069'
gh issue create --repo "$REPO" --title 'HU-070 · Crear una reserva sin posibilidad de overbooking' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend,clave" --body '**Como** recepcionista, **quiero** reservar un recurso para un periodo **para** vender la habitación con la certeza de que no está vendida ya
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-070'
gh issue create --repo "$REPO" --title 'HU-071 · Confirmar, cancelar y marcar no-show' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend,flutter" --body '**Como** recepcionista, **quiero** mover la reserva por sus estados **para** reflejar lo que de verdad pasó con el huésped
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-071'
gh issue create --repo "$REPO" --title 'HU-072 · Check-in con asignación de recurso' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend,flutter" --body '**Como** recepcionista, **quiero** hacer el check-in y asignar la habitación concreta **para** poder vender por tipo y decidir la habitación exacta al llegar el huésped
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-072'
gh issue create --repo "$REPO" --title 'HU-073 · Cargar consumos a la estancia' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend,flutter" --body '**Como** recepcionista, **quiero** cargar el minibar, el restaurante y la lavandería a la habitación **para** que el huésped pague todo junto al salir
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-073'
gh issue create --repo "$REPO" --title 'HU-074 · Check-out, liquidación y cierre de estancia' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,backend,flutter" --body '**Como** recepcionista, **quiero** liquidar la cuenta y cerrar la estancia **para** cobrar todo lo consumido y dejar la habitación lista para el siguiente
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-074'
gh issue create --repo "$REPO" --title 'HU-075 · Calendario visual de ocupación' --milestone 'E09 · Reservas · transacción del patrón Reserva' --label "reservas,flutter" --body '**Como** recepcionista, **quiero** ver la ocupación de todos los recursos en una línea de tiempo **para** entender de un vistazo cómo está la semana
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-075'
gh issue create --repo "$REPO" --title 'HU-076 · Cartas y categorías de menú por horario' --milestone 'E10 · Menú · catálogo del patrón Comanda' --label "menu,backend,flutter" --body '**Como** administrador del restaurante, **quiero** tener cartas distintas por franja horaria **para** no ofrecer desayunos a las ocho de la noche
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-076'
gh issue create --repo "$REPO" --title 'HU-077 · Ítems de menú con estación de cocina' --milestone 'E10 · Menú · catálogo del patrón Comanda' --label "menu,backend,flutter" --body '**Como** administrador del restaurante, **quiero** cargar los platos con su precio y la estación que los prepara **para** que cada comanda llegue a la parrilla o a la fría según corresponda
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-077'
gh issue create --repo "$REPO" --title 'HU-078 · Modificadores con mínimos y máximos' --milestone 'E10 · Menú · catálogo del patrón Comanda' --label "menu,backend,flutter" --body '**Como** administrador del restaurante, **quiero** definir opciones y adiciones por plato **para** poder pedir el término de la carne y cobrar el queso extra
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-078'
gh issue create --repo "$REPO" --title 'HU-079 · Recetas: el puente entre la comanda y el inventario' --milestone 'E10 · Menú · catálogo del patrón Comanda' --label "menu,backend,clave" --body '**Como** administrador del restaurante, **quiero** definir qué insumos consume cada plato **para** saber cuánta carne me queda sin contarla a mano
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-079'
gh issue create --repo "$REPO" --title 'HU-080 · Disponibilidad diaria de ítems' --milestone 'E10 · Menú · catálogo del patrón Comanda' --label "menu,backend,eventos" --body '**Como** jefe de cocina, **quiero** marcar un plato como agotado durante el servicio **para** que el mesero no lo siga vendiendo cuando ya no hay
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-080'
gh issue create --repo "$REPO" --title 'HU-081 · Zonas y mesas con su posición en el plano' --milestone 'E11 · Mesas · el salón' --label "mesas,backend,flutter" --body '**Como** administrador del restaurante, **quiero** dibujar mi salón con sus zonas y mesas **para** que el mesero vea el plano tal como es el local
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-081'
gh issue create --repo "$REPO" --title 'HU-082 · Sesión de mesa: abrir, ocupar y liberar' --milestone 'E11 · Mesas · el salón' --label "mesas,backend,flutter" --body '**Como** mesero, **quiero** abrir una mesa cuando llegan los comensales y liberarla cuando se van **para** saber cuáles están ocupadas y cuánto llevan
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-082'
gh issue create --repo "$REPO" --title 'HU-083 · Unir mesas para un grupo grande' --milestone 'E11 · Mesas · el salón' --label "mesas,backend,flutter" --body '**Como** mesero, **quiero** juntar dos o más mesas en una sola sesión **para** poder atender a un grupo de diez sin partir la cuenta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-083'
gh issue create --repo "$REPO" --title 'HU-084 · Plano del salón en tiempo real' --milestone 'E11 · Mesas · el salón' --label "mesas,flutter" --body '**Como** mesero, **quiero** ver el estado de todas las mesas de un vistazo **para** saber a dónde ir sin recorrer el local
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-084'
gh issue create --repo "$REPO" --title 'HU-085 · Abrir comanda y agregar líneas mientras el servicio avanza' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,flutter,clave" --body '**Como** mesero, **quiero** ir agregando platos a la comanda durante toda la comida **para** atender como se atiende de verdad: por rondas, no de una sola vez
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-085'
gh issue create --repo "$REPO" --title 'HU-086 · Ciclo de vida propio de cada línea' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,flutter" --body '**Como** mesero, **quiero** saber en qué va cada plato de la mesa **para** poder responder cuando el cliente pregunta cuánto falta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-086'
gh issue create --repo "$REPO" --title 'HU-087 · Anular una línea: antes y después de enviarla a cocina' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,clave" --body '**Como** mesero, **quiero** quitar un plato de la comanda **para** corregir un error sin que se pierda el rastro de lo que ya se cocinó
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-087'
gh issue create --repo "$REPO" --title 'HU-088 · Enviar a cocina y pantalla KDS por estación' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,flutter" --body '**Como** jefe de cocina, **quiero** ver en una pantalla lo que hay que preparar, agrupado por estación **para** trabajar sin papeles y saber qué se está demorando
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-088'
gh issue create --repo "$REPO" --title 'HU-089 · Dividir la cuenta entre comensales' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,flutter" --body '**Como** mesero, **quiero** partir la cuenta por ítem, en partes iguales o por monto **para** atender el «pagamos por separado» sin rehacer el pedido
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-089'
gh issue create --repo "$REPO" --title 'HU-090 · Cerrar la comanda con propina y descuento de insumos' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,backend,eventos,clave" --body '**Como** cajero, **quiero** cobrar la comanda y cerrarla **para** liberar la mesa y que el inventario refleje lo que se consumió
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-090'
gh issue create --repo "$REPO" --title 'HU-091 · Toma de comanda desde el celular del mesero' --milestone 'E12 · Comandas · transacción del patrón Comanda' --label "comandas,flutter" --body '**Como** mesero, **quiero** tomar el pedido en la mesa desde mi celular **para** no tener que ir al punto fijo a digitar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-091'
gh issue create --repo "$REPO" --title 'HU-092 · Motor de reglas de alerta configurable' --milestone 'E13 · Alertas y notificaciones' --label "alertas,backend" --body '**Como** administrador del negocio, **quiero** definir mis propias reglas de alerta **para** que alertar vencimientos en una droguería no sea un desarrollo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-092'
gh issue create --repo "$REPO" --title 'HU-093 · Alertas de inventario: stock bajo y lotes por vencer' --milestone 'E13 · Alertas y notificaciones' --label "alertas,backend" --body '**Como** administrador del negocio, **quiero** que me avisen antes de quedarme sin producto o de que se venza **para** no descubrirlo cuando el cliente ya lo está pidiendo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-093'
gh issue create --repo "$REPO" --title 'HU-094 · Entrega por push, correo y dentro de la app' --milestone 'E13 · Alertas y notificaciones' --label "alertas,backend,flutter" --body '**Como** usuario del negocio, **quiero** recibir las alertas por el canal que me sirva **para** enterarme aunque no tenga la app abierta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-094'
gh issue create --repo "$REPO" --title 'HU-095 · Centro de alertas en la app' --milestone 'E13 · Alertas y notificaciones' --label "alertas,flutter" --body '**Como** administrador del negocio, **quiero** ver todas mis alertas en un solo lugar **para** poder revisarlas cuando tenga tiempo, no solo cuando suenan
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-095'
gh issue create --repo "$REPO" --title 'HU-096 · Esquema en estrella alimentado por eventos' --milestone 'E14 · Reportes y dashboards' --label "reportes,backend,eventos,clave" --body '**Como** arquitecto, **quiero** que Reportes tenga su propio modelo dimensional que se llena con eventos **para** poder reportar sin consultar la base de ningún otro servicio
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-096'
gh issue create --repo "$REPO" --title 'HU-097 · Agregados diarios para el dashboard' --milestone 'E14 · Reportes y dashboards' --label "reportes,backend" --body '**Como** dueño del negocio, **quiero** ver mis ventas del día y del mes sin esperar **para** saber cómo va el negocio antes de cerrar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-097'
gh issue create --repo "$REPO" --title 'HU-098 · Reportes de ventas, márgenes y rotación' --milestone 'E14 · Reportes y dashboards' --label "reportes,backend,flutter" --body '**Como** dueño del negocio, **quiero** ver qué se vende, con qué margen y qué está quieto **para** decidir qué comprar y qué dejar de comprar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-098'
gh issue create --repo "$REPO" --title 'HU-099 · Métricas propias de Reserva y de Comanda' --milestone 'E14 · Reportes y dashboards' --label "reportes,backend,flutter" --body '**Como** dueño del hotel o del restaurante, **quiero** ver ocupación, ADR, RevPAR o rotación de mesas **para** medir mi negocio con los indicadores de mi industria, no con los de una tienda
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-099'
gh issue create --repo "$REPO" --title 'HU-100 · Exportar y programar reportes' --milestone 'E14 · Reportes y dashboards' --label "reportes,backend" --body '**Como** contador, **quiero** exportar un reporte y programar que llegue por correo **para** no tener que entrar cada lunes a sacar lo mismo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-100'
gh issue create --repo "$REPO" --title 'HU-101 · Bitácora de auditoría append-only' --milestone 'E15 · Auditoría y sincronización' --label "auditoria,backend" --body '**Como** auditor, **quiero** saber quién cambió qué y cuándo, en cualquier servicio **para** poder responder ante un reclamo o una revisión sin adivinar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-101'
gh issue create --repo "$REPO" --title 'HU-102 · Cola de sincronización de operaciones offline' --milestone 'E15 · Auditoría y sincronización' --label "auditoria,backend,flutter" --body '**Como** vendedor en ruta, **quiero** que todo lo que hice sin señal suba en orden y sin duplicarse **para** confiar en que nada de lo que registré se pierde
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-102'
gh issue create --repo "$REPO" --title 'HU-103 · Detección y resolución de conflictos de sincronización' --milestone 'E15 · Auditoría y sincronización' --label "auditoria,backend,flutter" --body '**Como** administrador del negocio, **quiero** saber cuándo dos personas cambiaron lo mismo y decidir qué queda **para** no perder trabajo en silencio
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-103'
gh issue create --repo "$REPO" --title 'HU-104 · Descarga incremental de cambios del servidor' --milestone 'E15 · Auditoría y sincronización' --label "auditoria,backend,flutter" --body '**Como** usuario de la app, **quiero** que la app baje solo lo que cambió desde la última vez **para** no gastar datos ni esperar a que baje todo el catálogo
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-104'
gh issue create --repo "$REPO" --title 'HU-105 · Consulta de auditoría desde la app' --milestone 'E15 · Auditoría y sincronización' --label "auditoria,flutter" --body '**Como** gerente, **quiero** ver el historial de cambios de un documento o de un usuario **para** aclarar una discusión sin llamar a soporte
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-105'
gh issue create --repo "$REPO" --title 'HU-106 · Tema y sistema de diseño en código' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core" --body '**Como** desarrollador, **quiero** tener los colores, tipografía y espaciado como constantes de Dart **para** que ninguna pantalla invente un hex a mano
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-106'
gh issue create --repo "$REPO" --title 'HU-107 · Navegación con rutas protegidas por rol y módulo' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core" --body '**Como** usuario del negocio, **quiero** que la app me lleve solo a donde puedo entrar **para** no toparme con pantallas que mi plan o mi rol no permiten
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-107'
gh issue create --repo "$REPO" --title 'HU-108 · Composición adaptativa entre móvil y escritorio' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core" --body '**Como** usuario del negocio, **quiero** que la app se vea bien en el celular y en el navegador ancho **para** trabajar en el mostrador y en la oficina con la misma herramienta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-108'
gh issue create --repo "$REPO" --title 'HU-109 · Sesión, token seguro y refresco automático' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core,seguridad" --body '**Como** usuario del negocio, **quiero** no tener que volver a entrar cada rato **para** trabajar sin que la app me saque a mitad de una venta
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-109'
gh issue create --repo "$REPO" --title 'HU-110 · Base de datos local con Drift en Android y Web' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core,clave" --body '**Como** desarrollador, **quiero** una capa de persistencia local que funcione igual en los dos destinos **para** que la app opere sin conexión con el mismo código
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-110'
gh issue create --repo "$REPO" --title 'HU-111 · Cola de sincronización en segundo plano' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core,clave" --body '**Como** vendedor en ruta, **quiero** que lo que registré sin señal suba solo cuando vuelva la conexión **para** no tener que acordarme de sincronizar
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-111'
gh issue create --repo "$REPO" --title 'HU-112 · Cliente HTTP con manejo uniforme de errores' --milestone 'E16 · App Flutter · núcleo' --label "flutter,core" --body '**Como** desarrollador, **quiero** un cliente HTTP que traduzca los códigos del backend a estados de la app **para** que un 402 se vea como «tu plan no incluye esto» y no como un error genérico
...
_Detalle completo en `historias/epicas/`._' >/dev/null && echo '  HU-112'

echo
echo "Listo: 112 historias en 17 hitos."
